...
describe('PatientController', () => {
  ...
  describe('createPatient', () => {
    it('should create patient and return the value', async () => {
      ...
      jest.spyOn(patientService, 'createPatient').mockResolvedValue(createPatientDto)

      const response = await controller.createPatient(createPatientDto, fileMock);
    ...
    it('should throw an HttpException when there is an error', async () => {
      ...
      jest.spyOn(patientService, 'createPatient').mockRejectedValue(new Error(errorMessage));

      await expect(controller.createPatient(createPatientDto, fileMock)).rejects.toThrow(new HttpException({ status: HttpStatus.BAD_REQUEST, message: errorMessage }, HttpStatus.BAD_REQUEST));
    });
  });
  ...
  describe('updatePatient', () => {
    it('should update patient and return updated patient', async () => {
      jest.spyOn(path, 'extname').mockReturnValue('.jpg');
      jest.spyOn(patientService, 'updatePatient').mockResolvedValue(updatePatientDto)

      const result = await controller.updatePatient(updatePatientDto._id, { fullname: 'ashir' }, fileMock);
  ...
  describe('findOnePatient', () => {
    ...
    it('should throw an HttpException when there is an error', async () => {
      ...
      jest.spyOn(patientService, 'findOne').mockRejectedValue(new Error(errorMessage));

      await expect(controller.findOnePatient(updatePatientDto._id)).rejects.toThrow(new HttpException({ status: HttpStatus.BAD_REQUEST, message: errorMessage }, HttpStatus.BAD_REQUEST));
    });
  });

  describe('updatePatientsSpecialAttention', () => {
    it('should update the patient to special attention', async () => {
      jest.spyOn(patientService, 'updatePatientSpecialAttention').mockResolvedValue(updatePatientDto)

      const result = await controller.updatePatientsSpecialAttention(updatePatientDto._id)
    ...
    it('should throw an HttpException when there is an error', async () => {
      const errorMessage = 'User Not found';
      jest.spyOn(patientService, 'updatePatientSpecialAttention').mockRejectedValue(new Error(errorMessage));

      await expect(controller.updatePatientsSpecialAttention(updatePatientDto._id)).rejects.toThrow(new HttpException({ status: HttpStatus.BAD_REQUEST, message: errorMessage }, HttpStatus.BAD_REQUEST));
    });
  });

  describe('deletePatient', () => {
    it('should delete the user and return 201', async () => {
      jest.spyOn(patientService, 'deletePatient').mockReturnValue(updatePatientDto);
    ...
    it('should throw an HttpException when there is an error', async () => {
      const errorMessage = 'User Not found';
      jest.spyOn(patientService, 'deletePatient').mockRejectedValue(new Error(errorMessage));

      await expect(controller.deletePatient(updatePatientDto._id)).rejects.toThrow(new HttpException({ status: HttpStatus.BAD_REQUEST, message: errorMessage }, HttpStatus.BAD_REQUEST));
    });
  });

  describe('findAll', () => {
    it('should return all the patient list', async () => {
      jest.spyOn(patientService, 'findAll').mockResolvedValue([{ ...updatePatientDto }]);
    ...
    it('should throw an HttpException when there is an error', async () => {
      const errorMessage = 'User Not found';
      jest.spyOn(patientService, 'findAll').mockRejectedValue(new Error(errorMessage));

      await expect(controller.findAll({})).rejects.toThrow(new HttpException({ status: HttpStatus.BAD_REQUEST, message: errorMessage }, HttpStatus.BAD_REQUEST));
    });
  })
});
