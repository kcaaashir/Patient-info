import { BadRequestException, Body, Controller, Delete, Get, HttpException, HttpStatus, Param, Post, Put, Query, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class PatientController {
    ...
    async createPatient(
        ...
        try {
            ...
            const patient = await this.patientService.createPatient(createPatientDto, file);
            return {
    ...
    async updatePatient(
        ...
        try {
            ...
            const patient = await this.patientService.updatePatient(id, updatePatientDto, file);
            return {
                status: HttpStatus.CREATED,
    ...
    async updatePatientsSpecialAttention(
        @Param('id') id: string,
    ) {
        try {
            const patient = await this.patientService.updatePatientSpecialAttention(id);
            return {
                status: HttpStatus.CREATED,
                message: 'A patient has been updated successfully',
                data: patient,
